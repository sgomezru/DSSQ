...
cfg.unet[DATA_KEY].training.batch_size = 35 if DATA_KEY == "heart" else 34
...
for iteration in range(5):
    ...
    for unet_name in ["monai-64-4-4", "swinunetr"]:
        ...
        # Hyperparams for adapter of ndims=2 and mode PCA is irrelevant for the
        # scalers training.
        adapters = [
            DimReductAdapter(
                swivel=swivel,
        ...
        unet_adapted = unet_adapted.to(device)
        unet_adapted.eval()
        ...
        for dim_red_mode in DIM_RED_MODES:
            for n_dims in N_DIMS:
                if dim_red_mode == "PCA" and n_dims != N_DIMS[-1]:
                    continue
                if dim_red_mode == "IPCA" and n_dims == N_DIMS[-1]:
                    continue
                adapters = [
                    DimReductAdapter(
                        swivel,
                ...
                list_n_dims = (
                    [2, 4, 8, 16, 512, 960] if dim_red_mode == "PCA" else [2, 4, 8, 16]
                )
                for n_dims in list_n_dims:
                    ...
                    unet_adapted.eval()
                    print(
                        f"Fitting gaussian for {dim_red_mode} module of {n_dims} dims"
                    )
                    for i, batch in enumerate(tqdm(dataloader)):
                        input_ = batch["input"].to(device)
                        unet_adapted(input_)
