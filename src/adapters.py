...
class DimReductAdapter(nn.Module):
    ...
    def _init(self):
        ...
        if self.pre_fit is False:
            ...
            if self.fit_gaussian is False: self._load_gaussian()
...
class DimReductModuleWrapper(nn.Module):
    ...
    def set_pre_fit_flag(self, val):
        assert isinstance(val, bool), 'Flag must be boolean'
        for adapter in self.adapters: adapter.pre_fit = val
        
    def set_fit_gaussian_flag(self, val):
        assert isinstance(val, bool), 'Flag must be boolean'
        for adapter in self.adapters: adapter.fit_gaussian = val

    def save_adapters_modules(self):
        for adapter in self.adapters: adapter._save_dim_red_module()

    def save_adapters_thresholds(self):
        for adapter in self.adapters: adapter._save_threshold()

    def fit_adapters_modules(self):
        for adapter in self.adapters: adapter._fit_dim_red_module()

    def fit_adapters_gaussians(self):
        for adapter in self.adapters: adapter._fit_gaussian()
